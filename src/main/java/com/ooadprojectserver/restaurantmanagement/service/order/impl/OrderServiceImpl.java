...
public class OrderServiceImpl implements OrderService {
    ...
    public void update(OrderRequest request) {
        Order updatingOrder = getOrderByTableIdAndStatus(request.getTableId(), request.getStatus());
        ...
        if (request.getOrderLines() != null && !request.getOrderLines().isEmpty()) {
            for (OrderLineRequest orderLineRequest : orderLineRequestList) {
                orderLineRepository.findByOrder_IdAndFood_Id(updatingOrder.getId(),
                        ...
                        orderLine1 -> orderLineService.update(orderLine1.getId(), orderLineRequest),
        ...
        updatingOrder.setCommonUpdate(userDetailService.getIdLogin());
    ...
    public void delete(UUID orderId) {
        ...
        // delete customer
        if (order.getCustomer().getVisitCount() == 0) {
            customerService.delete(order.getCustomer().getId());
        }
